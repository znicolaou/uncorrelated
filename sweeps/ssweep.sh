...
for ZGN_seed in `seq 1 $ZGN_Nseeds`; do

ZGN_delta=`bc -l <<< "$ZGN_delta1+(${ZGN_delta2} - ${ZGN_delta1})/${ZGN_Npoints}*${ZGN_temp}"`

echo ${i} ${ZGN_temp} ${ZGN_seed}

./stuartlandau 2 1.0,0.0 0.0,0.0 $ZGN_C $ZGN_delta 0 0 1 $ZGN_t1 $ZGN_t2 $ZGN_t3 $ZGN_dt $ZGN_dt2 $ZGN_seed rkf45 data/noise/uncorrelatedmult1 $ZGN_output &
